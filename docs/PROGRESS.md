...
- ✅ 将“应用到同族实例”预览改为显式确认弹窗：点击预览传播后居中展示影响范围，用户必须在弹窗中确认应用，避免预览内容落在正文底部被忽略。
- ✅ 统一正文 header 工具条观感：版本查看、对比、effective 状态、停用、预览传播、保存正文现在收敛为同一高度与圆角节奏，避免 label、badge、button 混排造成视觉割裂。
- ✅ 进一步压缩 Dashboard V3 技能详情空间：版本查看/对比控件已并入正文卡片 header，移除右侧独立版本历史模块，正文与 diff 区域恢复为单栏主工作区。
- ✅ 收拢 Dashboard V3 技能详情的版本历史区：右侧历史从长卡片列表改为“查看版本 / 对比版本”两个下拉控件，正文区在选择第二个版本后切换为代码风格 diff，支持单版本查看与双版本差异检查。
- 📝 前端经验：版本历史属于正文编辑的辅助控制，不应长期占据与正文等量的视觉权重；版本选择、对比选择、diff 呈现要拆成独立组件与 story 状态，避免以后再把长历史列表塞回主工作区。
...
- ✅ 锁定 `skills` 页结构轨道：技能库侧栏、详情主卡、正文编辑区、版本历史区不再按断点上下重排；窄屏改为页面横向滚动，长路径在固定 header 内压缩截断
- 📝 前端经验：结构锁定时左侧导航 rail 可以保持 `sticky`，但左右列宽和顺序必须由外层 grid 固定；sticky 只负责垂直跟随，不参与响应式重排决策
...
- ✅ 按 Storybook 标准重构 v3 stories：统一由 `preview` 承担路由、Frame、actions、a11y 和 story 排序；stories 改为状态导向写法，关键 Pattern stories 已补 `play` 交互，并新增 `docs/dashboard-v3-storybook.md` 作为 Story Map 与门禁文档
- ✅ 接入 Storybook 质量门：`frontend-v3` 已新增 `addon-vitest` 浏览器模式测试、Chromatic 视觉基线 workflow、根脚本 `test:storybook:dashboard-v3 / chromatic:dashboard-v3`，CI 现在会自动跑 Storybook component + a11y tests
...
- ✅ Dashboard V3 project 技能列表增加宿主筛选：默认按单一 host 展示技能，不再混排全部 runtime；表格移除 Runtime / Traces 列，新增基于 task episode scope 的评估次数列
- 📝 前端经验：项目视角里的 host 应作为列表范围控制，而不是每行重复信息；trace 数属于底层噪声，用户更需要看到“创建过多少次优化 scope / 评估过程”的产品指标
- ✅ Dashboard V3 skills 详情区融合 family 信息与正文编辑：同一个卡片内承载技能身份、项目/runtime、版本 diff、传播预览和保存动作，正文编辑器降级为嵌入式内容面板
- 📝 前端经验：同一任务路径里的身份信息、筛选条件和正文动作不要拆成两个等权模块；Storybook 需要在 Screen story 里锁定组合后的关键控件可见性与交互路径
