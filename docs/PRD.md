...
V2.0 的主界面采用 4 个一级页面，不再保留单独“主页”，而是把 skill、外部入口与项目工作现场并列成一级入口，统一收口到围绕 skill 生命周期管理的主工作流中:
...
3. `市场`
   - 面向外部 skill 供给的轻量入口
   - 第一阶段只聚合主流 skills 市场与 GitHub 资源库链接
   - 入口配置支持名称、链接、封面和标签；名称与站点 icon 可由 URL 自动推断，降低新增入口成本
   - 不抓取远端内容、不自动安装、不把外部 skill 混入本地 `Skill Family`
4. `配置`
   - 平台级配置能力
